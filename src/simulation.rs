...
const MERGE_SPAWN_BURST: usize = 8;
const MERGE_SPAWN_TOTAL_MASS_FRACTION: f32 = 0.006;
const MERGE_SPAWN_MIN_PARTICLE_MASS: f32 = 0.00002;
const MERGE_SPAWN_SPEED_FRAC: f32 = 0.28;
const MERGE_SPAWN_OFFSET_FRAC: f32 = 1.6;
        ...
        for action in actions {
            match action {
                ...
                ResolveAction::Merge {
                    ...
                    let burst_total_mass = (total_mass * MERGE_SPAWN_TOTAL_MASS_FRACTION)
                        .max(MERGE_SPAWN_MIN_PARTICLE_MASS * MERGE_SPAWN_BURST as f32);
                    let per_particle_mass = burst_total_mass / MERGE_SPAWN_BURST as f32;
                    if per_particle_mass >= MERGE_SPAWN_MIN_PARTICLE_MASS
                        && self.bodies[winner].mass > burst_total_mass + 0.01
                    {
                        self.bodies[winner].mass -= burst_total_mass;

                        let base_pos = self.bodies[winner].pos;
                        let base_vel = self.bodies[winner].vel;
                        let system_id = self.bodies[winner].system_id;
                        let burst_radius =
                            disk_spawn_radius_for_mass(per_particle_mass, 0.5).max(0.0004);
                        let offset = self.bodies[winner].radius.max(new_radius) * MERGE_SPAWN_OFFSET_FRAC;
                        let speed_boost = base_vel.mag() * MERGE_SPAWN_SPEED_FRAC;
                        let dirs = [
                        ...
                        for dir in dirs {
                            new_bodies.push(Body::new_with_system(
                                base_pos + dir * offset,
                                base_vel + dir * speed_boost,
                                per_particle_mass,
                                burst_radius,
                                system_id,
                            ));
